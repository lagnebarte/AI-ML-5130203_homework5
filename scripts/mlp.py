...
class MLPAlgorithm(object):
    ...
    def __init__(self, eta, threshold, max_epochs):
        self.eta = eta
        self.threshold = threshold #limit if conversion
        self.max_epochs = max_epochs #if not achieve threshold
    ...
    def build_architecture(self, input_length, hidden_length, output_length):
        ...
        self.input_length = input_length #how many inputs
        self.hidden_length = hidden_length #
        self.output_length = output_length
        
        # random initialization of weights and biases
        # hidden layer [b x (a+1)]
        self.Wh = np.random.rand(self.hidden_length, self.input_length)#like b and a
    ...
    # forward step
    def forward(self, x):  #step 3 in notebook  
        # hidden layer
        self.net_h = self.input_net(x, self.Wh, self.bh)
        self.fnet_h = np.array([self.f(net) for net in self.net_h]) #apply sigmoid def f
    ...
    def predict(self, xi):
        self.forward(xi)
        logits = self.fnet_o
        pred = np.where(self.fnet_o >= 0.5, 1, 0)
        
        return logits, pred.item() if pred.size == 1 else pred
    ...
    def fit(self, x_train, y_train):
        ...
        while(E >= self.threshold and count <= self.max_epochs + 1):
            ...
            for i in range(n):
                ...
                delta_h = self.dfnet_h * np.dot(delta_o, self.Wo) #deriweights
                ...
                for i in range(self.hidden_length):
                    ...
                    for k in range(self.input_length):
                        #dE_dWh
                        dE_dWh[i, k] = delta_h[i] * xi[k] #input
    ...
    def test(self, x_test, y_test):
        ...
        for i in range(n):
            ...
            y_pred.append(pred)  # pred теперь скаляр
            
            if pred == yi:
                self.accuracy += 1
        
        self.accuracy = 100 * (self.accuracy / n)
        return y_pred
